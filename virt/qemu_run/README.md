...
模拟器启动后，通过 hdc 命令行或 VNC 图形界面与客户机交互。连接端口由实例号与客户机架构决定（见「实例资源映射」），以下以默认实例 `00`、x86_64 客户机为例；其它实例号或架构的端口在实例资源映射表对应基址上偏移。
...
其它实例号或架构的 HDC 端口见「实例资源映射」，例如实例 `03` 的 HDC 端口为 `5558`。
...
VNC 默认仅监听 `127.0.0.1`，仅本机可达；在远程开发环境（VS Code / code-server 类 CodeSpace）中的远程访问方式见下节。其它实例号或架构的 VNC 端口见「实例资源映射」。
...
每个实例号具有固定端口。先使用 `status/list` 检查是否已有同号实例；选择其它实例号，不要手工修改端口与身份映射。
